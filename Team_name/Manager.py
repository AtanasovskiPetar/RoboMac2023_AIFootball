...
def team_properties():
    properties = dict()
    player_names = ["Messi", "Casemiro", "De Gea"]
    properties['team_name'] = "ManUtd"
...
def decision(our_team, their_team, ball, your_side, half, time_left, our_score, their_score):
    ...
    for p in players:
        ...
        if p == 'attacker':  # attacker
            ...
            if (your_side == 'left' and opponent_attacker['x'] < 683) or (
                    your_side == 'right' and opponent_attacker['x'] > 683):
                manager_decision[i]['alpha'] = get_angle_mid(player_x, player_y, opponent_attacker['x'],
                                                             opponent_attacker['y'], your_side)
                manager_decision[i]['shot_request'] = True
            # else:
            #     m = np.array([players['attacker']['x'], players['attacker']['y']])
            #     n = np.array([ball_x, ball_y])
            #     distance_attacker = np.linalg.norm(m - n)
            #     if distance_from_ball < distance_attacker:
            #         # manager_decision[i]['alpha'] = get_angle_attacker(player_x, player_y, ball_x, ball_y)
            #         if distance_from_ball < 50 and distance_from_goal < 300 and attacker_not_directed_at_goal(player,
            #                                                                                                   your_side,
            #                                                                                                   alpha):
            ...
            #             manager_decision[i]['shot_request'] = False
            #     else:
            #         manager_decision[i]['alpha'] = get_angle_mid_between(player, ball, their_team, your_side)
            #         manager_decision[i]['shot_request'] = False


            # if distance_from_ball < 50 and not attacker_not_directed_at_side(player, your_side, alpha, ball):
            else:
                # and ((opponent_attacker['x'] > 300 and your_side == 'left') or (opponent_attacker['x'] < 1066 and your_side == 'right')):
                if distance_from_ball < 50 and not attacker_not_directed_at_side(player, your_side, alpha, ball):
